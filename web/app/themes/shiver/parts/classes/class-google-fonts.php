...
if ( ! class_exists( 'shiver_Google_Fonts' ) ) :
	class shiver_Google_Fonts {
		...
		public static function get_google_fonts_url() {
			...
			$headings_font = get_theme_mod( 'shiver_headings_font', self::$default_headings_font );
			$body_font = get_theme_mod( 'shiver_body_font', self::$default_body_font );

			// Get the languages set in the customizer
			$font_languages = get_theme_mod( 'shiver_font_languages', array( 'latin' ) );
		...
		public static function get_font_value_from_name( $font_name, $font_option ) {
			...
			if ( $font_option == 'body' ) {
				$styles = apply_filters( 'shiver_google_font_body_styles', ':400,500,600,700,400italic,700italic' );
				...
				$headings_weight = get_theme_mod( 'shiver_headings_weight' );

				if ( $headings_weight && in_array( $headings_weight, $extra_weights ) ) {
					$styles .=  ',' . $headings_weight . ',' . $headings_weight . 'italic';
				}

				$styles = apply_filters( 'shiver_google_font_headings_styles', $styles );
			}
		...
		public static function get_suggested_fonts( $font_option ) {
			...
			return apply_filters( 'shiver_suggested_fonts', $suggested_fonts, $font_option );
		}
