...
if ( ! class_exists( 'shiver_Custom_CSS' ) ) :
	class shiver_Custom_CSS {
		...
		/*	-----------------------------------------------------------------------------------------------
			MINIFY CSS
			Helper function for reducing the size of the line styles output by shiver.
		...
		public static function get_customizer_css( $type = 'front-end' ) {
			...
			$body_font = 			esc_attr( get_theme_mod( 'shiver_body_font', shiver_Google_Fonts::$default_body_font ) );
			$headings_font = 		esc_attr( get_theme_mod( 'shiver_headings_font', shiver_Google_Fonts::$default_headings_font ) );
			$headings_weight =		esc_attr( get_theme_mod( 'shiver_headings_weight' ) );
			$headings_case =		esc_attr( get_theme_mod( 'shiver_headings_letter_case' ) );
			$headings_spacing =		get_theme_mod( 'shiver_headings_letterspacing' ) ? str_replace( '_', '.', esc_attr( get_theme_mod( 'shiver_headings_letterspacing' ) ) ) : ''; // Replace underscores with dots

			// Combine the chosen fonts with the appropriate fallback font stack
			if ( $body_font ) {
				$body_font_stack = 	shiver_Google_Fonts::get_font_fallbacks( $body_font, 'body' );
				$body_font = 		$body_font . ', '. $body_font_stack;
			}

			if ( $headings_font ) {
				$headings_font_stack = 	shiver_Google_Fonts::get_font_fallbacks( $headings_font, 'headings' );
			...
			$primary = 				get_theme_mod( 'shiver_primary_text_color' );
			$headings = 			get_theme_mod( 'shiver_headings_text_color' );
			$buttons_background = 	get_theme_mod( 'shiver_buttons_background_color' );
			$buttons_text = 		get_theme_mod( 'shiver_buttons_text_color' );
			$secondary = 			get_theme_mod( 'shiver_secondary_text_color' );
			$accent = 				get_theme_mod( 'shiver_accent_color' );
			$border = 				get_theme_mod( 'shiver_border_color' );
			$light_background = 	get_theme_mod( 'shiver_light_background_color' );
			$overlay_text = 		get_theme_mod( 'shiver_cover_template_overlay_text_color' );


			/* Properties -------------------- */

			// Make the list of CSS properties filterable
			$properties = apply_filters( 'shiver_css_properties', array(
				'body_font'				=> array(
					'default'				=> '',
					'value'					=> $body_font,
					'type'					=> 'font',
			...
			$output_p3 = apply_filters( 'shiver_output_p3_colors', true );
		...
		public static function get_css_elements_array( $type = 'front-end' ) {
			...
			switch( $type ) {
				case 'front-end' :
					$headings_targets = apply_filters( 'shiver_headings_targets_front_end', 'h1, h2, h3, h4, h5, h6, .faux-heading' );
					$buttons_targets = apply_filters( 'shiver_buttons_targets_front_end', 'button, .button, .faux-button, .wp-block-button__link, .wp-block-file .wp-block-file__button, input[type=\'button\'], input[type=\'reset\'], input[type=\'submit\']' );
					break;
				case 'block-editor' :
					$headings_targets = apply_filters( 'shiver_headings_targets_block_editor', '.editor-styles-wrapper .wp-block h1, .editor-styles-wrapper .wp-block h2, .editor-styles-wrapper .wp-block h3, .editor-styles-wrapper .wp-block h4, .editor-styles-wrapper .wp-block h5, .editor-styles-wrapper .wp-block h6, .editor-styles-wrapper .editor-post-title__block .editor-post-title__input' );
					$buttons_targets = apply_filters( 'shiver_buttons_targets_block_editor', '.editor-styles-wrapper .faux-button, .editor-styles-wrapper .wp-block-button:not(.is-style-outline) .wp-block-button__link, .editor-styles-wrapper .wp-block-file .wp-block-file__button' );
					break;
				case 'classic-editor' :
					$headings_targets = apply_filters( 'shiver_headings_targets_classic_editor', 'body#tinymce.wp-editor h1, body#tinymce.wp-editor h2, body#tinymce.wp-editor h3, body#tinymce.wp-editor h4, body#tinymce.wp-editor h5, body#tinymce.wp-editor h6' );
					$buttons_targets = apply_filters( 'shiver_buttons_targets_classic_editor', 'body#tinymce.wp-editor button, body#tinymce.wp-editor .button, body#tinymce.wp-editor .faux-button, body#tinymce.wp-editor input[type=\'button\'], body#tinymce.wp-editor input[type=\'reset\'], body#tinymce.wp-editor input[type=\'submit\']' );
					break;
			}
			...
			$elements = array(
				'front-end'			=> array(
					...
					'secondary'				=> array(
						...
						'color'					=> '.color-secondary, .color-secondary-hover:hover, :root .has-secondary-color, .wp-block-latest-comments time, .wp-block-latest-posts time',
					...
					'accent'				=> array(
						...
						'color'					=> '.color-accent, .color-accent-hover:hover, :root .has-accent-color, a, .wp-block-button.is-style-outline',
					...
					'buttons_background'	=> array(
						...
						'color'					=> ':root .has-buttons-background-color, .wp-block-button.is-style-outline',
					),
					'buttons_text'			=> array(
				...
				'block-editor'		=> array(
					...
					'primary'				=> array(
						...
						'color'					=> ':root .has-primary-color, .editor-styles-wrapper > *, .editor-styles-wrapper .editor-post-title__input, .editor-styles-wrapper .is-style-outline .wp-block-button__link',
					),
					'headings'				=> array(
						'color'					=> $headings_targets,
					),
			...
			return apply_filters( 'shiver_get_css_elements_array', $elements[$type], $type );
