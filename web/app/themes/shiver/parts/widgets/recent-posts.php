...
class shiver_Recent_Posts extends WP_Widget {

	function __construct() {
		$widget_ops = array(
			'classname' 	=> 'widget_shiver_recent_posts',
			'description' 	=> __( 'Displays recent blog entries with post thumbnails.', 'shiver' ),
		);
		parent::__construct( 'widget_shiver_recent_posts', __( 'Recent Posts', 'shiver' ), $widget_ops );
	}
				...
				<?php foreach ( $recent_posts as $post ) :
					...
					<li>
						...
						<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
							...
							$image_url = $image ? $image[0] : shiver_get_fallback_image_url();
							?>
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'widget_title' ); ?>"><?php  esc_html_e( 'Title', 'shiver' ); ?>:
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'number_of_posts' ); ?>"><?php esc_html_e( 'Number of posts to display', 'shiver' ); ?>:
			...
			<small>(<?php esc_html_e( 'Defaults to 3 if empty', 'shiver' ); ?>)</small>
		</p>
